...
- User has confirmed authorization in the current session with a narrow execution phrase, chat confirmation, or host-native execution decision scoped to this batch
...
2. If the batch is dry-run and the user did not explicitly authorize execution in the current turn, ask once using host-native decision/input UI when available:
   - Question: "Execute this batch now?"
   - Options: `execute_now`, `keep_dry_run`
   - Include the batch path, selected task IDs, parallel groups, subagent dispatch expectation, and validation commands
   - If the user selects `execute_now`, continue to the dry-run upgrade checks
   - If the user declines or no interactive input is available, refuse with the message below and stop:
     ```
     Cannot execute: batch is dry-run only.
     To execute this exact batch, use the execution decision prompt or explicitly say "execute this batch" or "implement this wave".
     ```
3. If the batch is dry-run and the user explicitly authorized execution by decision, chat confirmation, or phrase, run the dry-run upgrade checks:
   ...
   - If all checks pass, update the batch artifact's authorization section to `Mode: execute`, `Approved by user: yes`, `Authorization source: decision-ui | chat-confirmation | phrase`, current timestamp, and `Upgraded by batch-to-build: yes`
4. If the batch is already execute and the user has not confirmed in this session: ask once using host-native decision/input UI when available. If user confirms, proceed. Otherwise refuse.
5. If user has confirmed: proceed.
6. If any parallel group contains 2+ tasks, confirm the execution mode is subagent-driven. If the batch artifact does not explicitly say this, treat it as an execution-contract defect and update the artifact before proceeding.
